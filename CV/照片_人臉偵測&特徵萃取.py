...
img = imutils.resize(img, width=1280)     # 縮小圖片，亦可直接使用原圖


# 呼叫Dlib 的人臉偵測器 & 特徵萃取器
'''Dlib 使用的人臉偵測演算法是以方向梯度直方圖（HOG）的特徵加上線性分類器（linear classifier）、影像金字塔（image pyramid）與滑動窗格（sliding window）來實作。'''
...
for face in face_rects:
    ...
    # 寫入照片
    cv2.imwrite('Facial Landmarks'', img)
